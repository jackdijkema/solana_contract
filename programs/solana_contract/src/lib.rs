...
pub mod solana_contract {
  ...
  pub fn initialize(ctx: Context<Initialize>, withdraw_time: i64) -> ProgramResult {
    ...
    msg!("Initialized user data with withdrawal time set to {}", withdraw_time);
    Ok(())
}

    pub fn deposit_token(ctx: Context<DepositToken>, amount: u64, withdraw_time: i64, mint: Pubkey) -> ProgramResult {
        ...
        msg!("Deposited {} tokens into vault at {:?} with timestamp {}", amount, ctx.accounts.vault_token_account.key(), ctx.accounts.user_data.withdrawal_time);
